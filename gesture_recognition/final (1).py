...
def range_detection():
    ...
    h_full = 479
    w_full = 639
    quad = [[(0,h_limit+60),(w_limit,h_limit+60),(w_limit,h_full - h_limit),(0,h_full)],#right
            [(w_full - w_limit,h_limit+60),(w_full,h_limit+60),(w_full,h_full),(w_full - w_limit,h_full - h_limit)],#left
            [(w_limit,0),(w_full - w_limit,0),(w_full - w_limit,h_limit),(w_limit,h_limit)],#up
            [(0,h_full),(w_limit,h_full - h_limit),(w_full - w_limit,h_full - h_limit),(w_full,h_full)],#down
            [(w_full - w_limit,h_limit + 60),(w_full,h_limit + 60),(w_full,0),(w_full - w_limit,0)],#roatation left
            [(0,0),(w_limit,0),(w_limit,h_limit + 60),(0,h_limit+60)],#rotation right
            [(w_limit,h_limit),(w_full - w_limit,h_limit),(w_full - w_limit,h_full - h_limit),(w_limit,h_full - h_limit)]]#centre
    pos = ["Right","Left","Up","Down","Rotation_Left","Rotation_Right","Centre"]
    tipIds = [4, 8, 12, 16, 20]
    command = {"Stay":1,"Left":2,"Right":3,"Up":4,"Down":5,"Forward":6,"Back":7,"Rotation_Left":8,"Rotation_Right":9} #command Mapping
    yo = [1, 1, 0, 0, 1]
    while True:
        move = ""
        success, img = cap.read()
        ...
        cv2.line(img,(0,h_limit+60),(w_limit, h_limit+60),(250, 44, 250),2)
        cv2.line(img,(w_limit,0),(w_limit, h_limit+60),(250, 44, 250),2)
        cv2.line(img,(w_full - w_limit, h_full - h_limit),(w_full,h_full),(250, 44, 250))
        cv2.line(img,(w_full - w_limit, h_limit + 60),(w_full, h_limit + 60),(250, 44, 250),2)
        cv2.line(img,(w_full - w_limit, h_limit + 60),(w_full - w_limit,0),(250, 44, 250),2)
